...
// ======================================================
// ==       SKETSA TES BUZZER BERKEDIP (NON-BLOCKING)  ==
// ======================================================

// Atur pin buzzer sesuai dengan koneksi Anda
const int BUZZER_PIN = 18;

void setup() {
  Serial.begin(115200);
  pinMode(BUZZER_PIN, OUTPUT);
  Serial.println("Memulai tes buzzer berkedip...");
}

void loop() {
  // Baris ini akan membuat pin buzzer beralih antara HIGH dan LOW
  // setiap 500 milidetik (setengah detik) secara terus-menerus.
  digitalWrite(BUZZER_PIN, (millis() / 500) % 2);

  // Kita bisa menambahkan Serial.print untuk melihat nilai digitalnya
  // Hapus komentar di bawah ini jika ingin melihat outputnya
  /*
  int buzzerState = (millis() / 500) % 2;
  Serial.print("Status Pin Buzzer: ");
  Serial.println(buzzerState); // Akan menampilkan 1, 0, 1, 0, ...
  delay(100); // Beri jeda agar serial monitor tidak terlalu cepat
  */
}